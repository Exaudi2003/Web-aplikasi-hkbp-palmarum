...
{
    $sentralisasi = Http::get('http://127.0.0.1:8070/api/setSentralisasi')->collect();
    $sentralisasiArray = $sentralisasi->toArray(); // Convert the collection to an array

    $data = array_merge($sentralisasiArray);

    return view('layouts.forms.setSentralisasi')->with('sentralisasi', $sentralisasi);
}
    ...
    public function create(Request $req)    
    {
        ...
        if (!$res->failed()) {
            return back()->withErrors(['message' => 'Error when creating Sentralisasi']);
        }
